...
- NEW: Patch coloring mode - shows patches with unique colors and legend (DEFAULT ON)
"""
...
class Tab5HexPatches:
    ...
    def setup_ui(self):
        ...
        tk.Label(header_frame, text="5. Patches & Assignment", 
                 font=("Segoe UI", 14, "bold"),
                 bg=self.colors['bg'], fg=self.colors['fg']).pack(side=tk.LEFT)
        tk.Label(header_frame, text="3D Hex Block View - Click faces to select/hide", 
        ...
        controls_frame = tk.Frame(self.canvas, bg=self.colors['secondary'],
                                  highlightbackground=self.colors['border'], highlightthickness=1)
        controls_frame.place(x=10, y=10)  # float inside canvas

        # NEW: Patch coloring toggle button
        self.patch_coloring_button = tk.Button(controls_frame, text="🎨 Patches: ON", 
                 command=self._toggle_patch_coloring,
                 bg=self.colors['success'], fg=self.colors['bg'],
                 font=("Arial", 9, "bold"), relief=tk.FLAT,
                 activebackground='#3db89f', width=14)
        self.patch_coloring_button.pack(side=tk.LEFT, padx=5, pady=2)
        ...
        self.mode_button = tk.Button(controls_frame, text="Mode: Select", 
                 ...
                 font=("Arial", 9, "bold"), relief=tk.FLAT,
                 activebackground=self.colors['button_active'], width=12)
        ...
        self.show_all_button.pack(side=tk.LEFT, padx=5, pady=2)
        ...
        info_label = tk.Label(self.canvas, 
                ...
                bg=self.colors['secondary'], highlightbackground=self.colors['border'], highlightthickness=1)
        info_label.place(x=10, rely=1.0, y=-10, anchor="sw")
    ...
    def _on_patch_editor_save(self):
        ...
        if self.renderer:
            ...
            self.renderer.invalidate_cache()  # Rebuild patch mapping
            self.renderer.draw()
    ...
    # NEW: Toggle patch coloring mode
    def _toggle_patch_coloring(self):
        """Toggle patch coloring mode on/off"""
        if self.renderer:
            is_enabled = self.renderer.toggle_patch_coloring()
            if is_enabled:
    ...
    def _init_renderer(self):
        ...
        # Update patch assignment panel with renderer reference
