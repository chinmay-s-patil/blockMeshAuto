...
class EmbeddedViewer:
    ...
    def _get_3d_coords_from_mesh(self, layer_name, local_idx):
        """
        Get 3D coordinates from mesh_data using global point ID.
        Returns standard (X, Y, Z) format.
        """
        # Get the point ID from the layer
        layer_data = self.mesh_data.layers.get(layer_name, {})
        point_refs = layer_data.get('point_refs', [])
        
        if local_idx < 0 or local_idx >= len(point_refs):
            return np.array([0, 0, 0])
        
        point_id = point_refs[local_idx]
        point_data = self.mesh_data.get_point(point_id)
        
        if point_data is None:
            return np.array([0, 0, 0])
        
        # Return standard (X, Y, Z) - no swapping needed as get_point returns correct coords
        return np.array([point_data['x'], point_data['y'], point_data['z']])
    ...
    def _rebuild_coord_cache(self):
        ...
        for layer_name in layer_list:
            ...
            for local_idx, point_id in enumerate(point_refs):
                point_data = self.mesh_data.get_point(point_id)
                if point_data:
    ...
    def _draw_hex_blocks(self):
        ...
        # Access hex blocks from mesh_data
        ...
        for block_id, block in hex_blocks.items():
            ...
            for point_id in point_refs:
                ...
                if point_data is None:
                    valid = False
                    break
                vertices.append(np.array([point_data['x'], point_data['y'], point_data['z']]))
            
            if not valid or len(vertices) != 8:
                continue
